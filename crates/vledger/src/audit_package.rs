...
pub fn generate(data_dir: &Path, output_path: &Path, opts: GenerateOptions) -> Result<GenerateReport> {
    ...
    ledger.verify_chain_integrity()
        .context("Cannot generate audit package: ledger hash chain is broken. \
...
pub fn prove_entry(
    ...
    let commitment: Value = serde_json::from_str(&raw)
        .context("Invalid JSON in commitment file")?;
    let committed_root = commitment["meta"]["merkle_root"].as_str()
    ...
    let idx = entries.iter().position(|e| e.sequence == sequence)
    ...
    let json_str = serde_json::to_string_pretty(&proof_json)
        .context("Failed to serialize entry proof")?;
    std::fs::write(output_path, &json_str)
        .with_context(|| format!("Cannot write to {}", output_path.display()))?;
...
pub fn verify(file: &Path) -> Result<VerifyReport> {
    ...
    if package_type == "entry_proof" {
        entry_count = 1;
        if let Ok(entry) = serde_json::from_value::<vledger_ledger::entry::JournalEntry>(
            package["entry"].clone()
        ) {
            // Content hash
            ...
            let leaf_bytes_opt: Option<[u8; 32]> = hex::decode(leaf_hex).ok()
    ...
    if package_type == "full" {
        ...
        if let Some(entries_arr) = entries_json {
            ...
            let entries: Vec<vledger_ledger::entry::JournalEntry> = entries_arr.iter()
                .enumerate()
                .filter_map(|(i, v)| {
            ...
            let computed_root = if leaf_bytes.is_empty() { ZERO_HASH } else { merkle_root(&leaf_bytes) };
            ...
            if let Some(proofs) = proofs_json {
                ...
                for (i, proof_val) in proofs.iter().enumerate() {
                    let leaf_hex      = proof_val["leaf_hash"].as_str().unwrap_or("");
                    let root_in_proof = proof_val["root"].as_str().unwrap_or("");
                    let leaf_opt: Option<[u8; 32]> = hex::decode(leaf_hex).ok()
                        .and_then(|b| b.try_into().ok());
                        ...
                        if let Some(steps) = proof_val["path"].as_array() {
                            for step in steps {
                                ...
                                let sib: Option<[u8; 32]> = hex::decode(sib_hex).ok()
                                    .and_then(|b| b.try_into().ok());
                                if let Some(s) = sib {
                                    cur = if sib_is_left { hash_node(&s, &cur) } else { hash_node(&cur, &s) };
                                }
                            }
                        }
                        if hash_to_hex(&cur) != root_in_proof {
                ...
                if merkle_errors > 0 { merkle_ok = false; }
            }
        }
    }
