...
mod tests {
    ...
    fn mk_accounts(store: &mut LedgerStore) -> (uuid::Uuid, uuid::Uuid) {
        ...
        let r = store.create_account(Account::new("REV", "Rev", AccountType::Income, "USD", "test")).unwrap();
        (c, r)
    }
    ...
    fn stage3_20_crash_reopen_cycles_monotonically_correct() {
        ...
        for i in 1u64..=20 {
            ...
            assert!(s2.verify_chain_integrity().is_ok(),
    ...
    fn stage4_reversal_atomicity_survives_crash() {
        ...
        {
            let mut s = mk_store(&dir);
            s.reverse_entry(original_id, "reversal test", "test").unwrap();
        } // simulated crash after commit

        // Reopen and verify: reversal must be fully committed
        let s = mk_store(&dir);
        assert_eq!(s.entry_count(), 2, "original + reversal must both be present");
        assert_eq!(s.balance(&cash), 0, "reversal must zero out the balance");
        assert!(s.verify_chain_integrity().is_ok(), "hash chain must be valid");
    ...
    fn stage5_each_entry_is_an_independent_crash_unit() {
        ...
        assert_eq!(s.balance(&cash), (1i128..=10).map(|i| i * 500).sum::<i128>());
    ...
    fn stage6_idempotency_key_persists_across_crash() {
        ...
        // Reopen and attempt to re-post with the same key
        {
            ...
            assert!(result.is_ok(), "idempotent re-post must succeed, got: {result:?}");
